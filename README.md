...
OpenClaw 是 SAP Concur 的**前置智能审核层**，面向在华跨国企业（医药、零售等）。

**核心定位**：员工报销的第一个 contact point — 审核通过后才进入 Concur 审批流。

```
员工 ──► OpenClaw（OCR + 规则校验 + AI 审核）──► Concur（审批流）
              ↑                                        │
              └──── 拉取结构化数据做事后审计 ◄───────────┘
```
...
| Concur 能做的 | OpenClaw 做的（Concur 做不了的） |
|---|---|
...
| 03 | [Integrated Flow Diagram](docs/03-integrated-flow-diagram.md) | 端到端集成流程图：员工→OpenClaw→Concur→事后审计 |
...
*OpenClaw — SAP Concur Intelligent Audit Enhancement Layer*
